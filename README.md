...
通常モードでは Deep SORT が選んだ main target の bbox と姿勢推定結果を対応付けてズームする。`--fast` モードでは YOLO11-Pose の full-frame 検出結果から最大 bbox の人物を主対象にする。対象を見失った場合は、最大 30 フレームは最後の位置を維持し、その後は画面中央・等倍に戻る。
