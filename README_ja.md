...
本プロジェクトは、開発者自身が抱える「**基礎スキー**（フォームの美しさと技術点を競う競技種目）で客観的に自己評価する手段がない」という当事者課題を出発点に、コンピュータビジョンで解こうとした個人開発です。技術力だけでなく、**制約を見極めて価値を出すまでの判断**を見ていただけると思います。

- **製品的判断** — 当初は「AI による自動採点」を目指しましたが、映像単体では斜度や雪面状況が取得できず、検定基準の再現には学習データも不足することを早期に見極めました。完走を優先して中途半端な採点機能を作るのではなく、スコープを **「自動採点」から「姿勢の可視化と定量評価」へ再定義**し、確実に実用価値の出る形に切り替えています。
- **エンジニアリング** — YOLOv8x（検出）+ SAM 3D Body / YOLO11-Pose（姿勢）+ Deep SORT（追跡）を統合。姿勢推定エンジンは `PoseBackend` 抽象基底クラスで差し替え可能にし、`.env` 一つで 2D 軽量モデルと 3D 高精度モデルを切り替えられる拡張設計にしています。描画で多発した座標系のズレは `transform_point_to_zoom()` への一元化で解消しました。
- **ドメイン × 技術** — 基礎スキー競技者としての知見を評価ロジック（膝・股関節・足首・肩の角度基準）に落とし込み、視点に依存しない 3D 関節角度で前傾・内傾・左右対称性を定量化しています。
- **実用性** — 開発者自身が日々の練習映像の振り返りに実際に使用しており、継続的に改善中です。

技術的な詳細・設計判断・苦労した点の全文は [`docs/project_details_ja.md`](docs/project_details_ja.md) を参照してください。
